...
for a in "${apis[@]}"; do
  ...
  if [ ! -f "$info" ]; then
    if az apim api show -g "$RG" --service-name "$APIM" --api-id "$id" -o none 2>/dev/null; then echo "  FAIL $a still exists in APIM after removal"; fail=1; else echo "  PASS $a removed"; fi; continue
  fi
