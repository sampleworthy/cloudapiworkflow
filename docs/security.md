...
* Agent tool calls are signed by the Foundry account's managed identity (observed at the gateway), which holds only read app roles granted by Terraform; each agent's own Entra Agent ID identity is granted the roles its definition declares (allow-listed by the platform) for when Foundry signs with it. APIM validates the calls exactly like any client (identity, roles) and attributes them by `X-Caller-Id`.
