...
* Each agent runs as its own Entra Agent ID identity with read-only app roles declared in its definition and allow-listed by the platform; its tool calls are validated by APIM exactly like any client (identity, roles) and attributed by `X-Caller-Id`.
